...
class ValidInputTest(unittest.TestCase):
    def setUp(self):
        self.images = [
            TestInfo(
                "images/pydis_logo_scrambled.png",
                (512, 512),
                (256, 256),
                "images/pydis_logo_order.txt",
                "images/pydis_logo_unscrambled.png",
            ),
            TestInfo(
                "images/great_wave_scrambled.png",
                (1104, 1600),
                (16, 16),
                "images/great_wave_order.txt",
                "images/great_wave_unscrambled.png",
            ),
            TestInfo(
                "images/secret_image1_scrambled.png",
                (800, 600),
                (20, 20),
                "images/secret_image1_order.txt",
                "images/secret_image1_unscrambled.png",
            ),
            TestInfo(
                "images/secret_image2_scrambled.png",
                (800, 600),
                (20, 20),
                "images/secret_image2_order.txt",
    ...
    def test_tile_size_doesnt_match_image_size(self):
        ...
        for tile_size in test_cases:
            with self.subTest(tile_size=tile_size):
                self.assertFalse(
                    qualifier.valid_input(
                        self.images[0].image_size, tile_size, self.images[0].ordering
                    )
                )
    ...
    def test_invalid_ordering(self):
        ...
        ordering[-1] -= 1
        self.assertFalse(
            qualifier.valid_input(self.images[0].image_size, (256, 256), ordering)
        )
    ...
    def test_tile_size_doesnt_match_ordering(self):
        ...
        ordering.append(
            len(ordering)
        )  # Add this value specifically so the ordering itself remains valid.
        self.assertFalse(
            qualifier.valid_input(self.images[0].image_size, (256, 256), ordering)
        )
    ...
    def test_valid_input(self):
        ...
        for image_size, tile_size, ordering in test_cases:
            with self.subTest(
                image_size=image_size, tile_size=tile_size, ordering=ordering
            ):
    ...
    def test_valid_input_called(self):
        ...
        qualifier.rearrange_tiles(
            self.images[0].unscrambled_image_path,
            self.images[0].tile_size,
            self.images[0].ordering,
        ...
        for image_index in range(len(self.images)):
            with self.subTest(image_index=image_index):
                ...
                correct_output = np.array(
                    Image.open(self.images[image_index].unscrambled_image_path)
                )
